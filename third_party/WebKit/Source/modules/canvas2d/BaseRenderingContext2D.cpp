...
void BaseRenderingContext2D::realizeSaves() {
  ...
  if (state().hasUnrealizedSaves()) {
    ...
    CdlCanvas* canvas = drawingCanvas();
    if (canvas)
      canvas->save();
    validateStateStack();
  }
...
void BaseRenderingContext2D::restoreMatrixClipStack(CdlCanvas* c) const {
  if (!c)
    return;
...
void BaseRenderingContext2D::fillRect(double x,
  ...
  draw([&rect, this](CdlCanvas* c, const CdlPaint* paint)  // draw lambda
...
void BaseRenderingContext2D::clearRect(double x,
  ...
  clearPaint.setStyle(CdlPaint::kFill_Style);
...
void BaseRenderingContext2D::drawImageInternal(CdlCanvas* c,
...
void BaseRenderingContext2D::checkOverdraw(
  ...
  if (paint) {
    ...
    if (isSourceOver && imageType == CanvasRenderingContext2DState::NoImage) {
      CdlShader* shader = paint->getShader();
      if (shader) {
