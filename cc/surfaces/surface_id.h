// Copyright 2014 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#ifndef CC_SURFACES_SURFACE_ID_H_
#define CC_SURFACES_SURFACE_ID_H_

#include <stddef.h>
#include <stdint.h>

#include <functional>

#include "base/format_macros.h"
#include "base/hash.h"
#include "base/strings/stringprintf.h"

namespace cc {

class SurfaceId {
 public:
  SurfaceId() : client_id_(0), sink_id_(0), local_id_(0), nonce_(0) {}

  SurfaceId(const SurfaceId& other)
      : client_id_(other.client_id_),
        sink_id_(other.sink_id_),
        local_id_(other.local_id_),
        nonce_(other.nonce_) {}

  // A SurfaceId consists of four components:
  //    client Id, sink Id, local Id, and nonce.
  // A |client_id| is a display compositor service allocated ID that
  // uniquely identifies a client.
  // A |sink_id| uniquely identifies a CompositorFrameSink within a client.
  // A |local_id| is a sequentially allocated ID generated by the display
  // compositor client that uniquely identifies a surface.
  // A |nonce| is a cryptographically secure random int that makes a SurfaceId
  // unguessable by other clients.
  SurfaceId(uint32_t client_id,
            uint32_t sink_id,
            uint32_t local_id,
            uint64_t nonce)
      : client_id_(client_id),
        sink_id_(sink_id),
        local_id_(local_id),
        nonce_(nonce) {}

  bool is_null() const {
    return client_id_ == 0 && sink_id_ == 0 && nonce_ == 0 && local_id_ == 0;
  }

  size_t hash() const {
    size_t interim = base::HashInts(client_id_, sink_id_);
    size_t interim2 = base::HashInts(local_id_, nonce_);
    return base::HashInts(interim, interim2);
  }

  uint32_t client_id() const { return client_id_; }

  uint32_t sink_id() const { return sink_id_; }

  uint32_t local_id() const { return local_id_; }

  uint64_t nonce() const { return nonce_; }

  std::string ToString() const {
    return base::StringPrintf("%d:%d:%d:%" PRIu64, client_id_, sink_id_,
                              local_id_, nonce_);
  }

  bool operator==(const SurfaceId& other) const {
    return client_id_ == other.client_id_ && sink_id_ == other.sink_id_ &&
           local_id_ == other.local_id_ && nonce_ == other.nonce_;
  }

  bool operator!=(const SurfaceId& other) const { return !(*this == other); }

  bool operator<(const SurfaceId& other) const {
    return std::tie(client_id_, sink_id_, local_id_, nonce_) <
           std::tie(other.client_id_, other.sink_id_, other.local_id_,
                    other.nonce_);
  }

 private:
  // See SurfaceIdAllocator::GenerateId.
  uint32_t client_id_;
  uint32_t sink_id_;
  uint32_t local_id_;
  uint64_t nonce_;
};

struct SurfaceIdHash {
  size_t operator()(const SurfaceId& key) const { return key.hash(); }
};

}  // namespace cc

#endif  // CC_SURFACES_SURFACE_ID_H_
